...
def Nontransformation(df, df2: DataFrame, Log=False, Exponential=False, Quadratic=False, Reciprocal=False):
    ...
    for variable in variable_fields:
        if Exponential:
            df["TRANS"] = df[variable].apply(np.exp)
            df.insert(0, "TRANS", df.pop("TRANS"))
        elif Quadratic:
            df["TRANS"] = df[variable].apply(lambda x: x**2)
            df.insert(0, "TRANS", df.pop("TRANS"))
        elif Reciprocal:
            df["TRANS"] = df[variable].apply(lambda x: 1/x if x != 0 else np.nan)
            df.insert(0, "TRANS", df.pop("TRANS"))
